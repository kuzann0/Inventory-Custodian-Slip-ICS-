...
class DynamicDataBinding {
    ...
    public function getWorkflowEntry($pr_id) {
        try {
            ...
            // Parse JSON fields
            $entry['step_1_data'] = json_decode($entry['step_1_data'], true);
            $entry['step_2_data'] = json_decode($entry['step_2_data'], true);
            $entry['step_3_data'] = json_decode($entry['step_3_data'], true);
            $entry['step_4_data'] = json_decode($entry['step_4_data'], true);
            $entry['step_5_data'] = json_decode($entry['step_5_data'], true);

...
-- Fixed: 2026-05-05 (Final MySQL 5.7 compatibility pass)
--
-- FIXES APPLIED:
-- 1. DROP PROCEDURE IF EXISTS for MySQL 5.7 compatibility (no CREATE IF NOT EXISTS)
-- 2. Helper procedures for safe column/index additions (idempotent)
-- 3. Stored procedures check INFORMATION_SCHEMA before modifications
-- 4. Fixed spacing inconsistencies in column definitions
-- 5. Added missing idx_entry_id index to entry_workflow_status
-- 6. INSERT IGNORE to prevent duplicate key errors (#1062)
-- 7. Full CREATE TABLE IF NOT EXISTS protection
--
-- COMPATIBILITY:
-- ✓ MySQL 5.7 (tested and verified)
-- ✓ MySQL 8.0+ (fully compatible)
-- ✓ Idempotent - safe to run multiple times
-- ✓ No errors on duplicate columns or indexes
--
-- PREREQUISITES:
-- - Ensure entries table exists with order_id as primary key
-- - Ensure purchase_requests table exists with id as primary key
-- - Database user has ALTER TABLE and CREATE PROCEDURE permissions
-- - Backup database before running in production
--
-- EXECUTION NOTES:
-- - Run this script once; it is now safe to re-run
-- - Procedure automatically skips existing columns
-- - Monitor for FK constraint errors (dependencies on parent tables)
-- - Verify with queries in section 9
-- =====================================================

-- =====================================================
-- 0. CREATE HELPER PROCEDURES - Safe schema modifications
-- =====================================================
-- These procedures check if columns/indexes exist before adding them
-- Prevents errors on re-runs (MySQL 5.7 compatible)
-- Uses DROP IF EXISTS for 5.7 compatibility (CREATE IF NOT EXISTS added in 8.0)
...
CREATE PROCEDURE add_index_if_not_exists(
  ...
  p_index_definition VARCHAR(500)
)
BEGIN
  ...
  IF idx_count = 0 THEN
    SET @sql = CONCAT('ALTER TABLE ', p_table_name, ' ADD INDEX ', p_index_definition);
    PREPARE stmt FROM @sql;
    EXECUTE stmt;
    DEALLOCATE PREPARE stmt;
  END IF;
...
-- Uses helper procedure to safely add columns (idempotent)

CALL add_column_if_not_exists('entries', 'ApprovalStatus', '`ApprovalStatus` VARCHAR(50) DEFAULT NULL COMMENT "Approval status (pending, approved, rejected)"');
CALL add_column_if_not_exists('entries', 'ApprovedBy', '`ApprovedBy` INT(11) DEFAULT NULL COMMENT "User ID who approved"');
CALL add_column_if_not_exists('entries', 'ApprovedDate', '`ApprovedDate` TIMESTAMP NULL DEFAULT NULL COMMENT "When approved"');

CALL add_column_if_not_exists('entries', 'DeliveryNotes', '`DeliveryNotes` TEXT DEFAULT NULL COMMENT "Delivery notes from Step 3"');
CALL add_column_if_not_exists('entries', 'DeliveryDate', '`DeliveryDate` DATE DEFAULT NULL COMMENT "Expected delivery date"');
CALL add_column_if_not_exists('entries', 'DeliveryStatus', '`DeliveryStatus` VARCHAR(50) DEFAULT NULL COMMENT "Delivery status (pending, in_transit, delivered)"');
CALL add_column_if_not_exists('entries', 'ActualDeliveryDate', '`ActualDeliveryDate` DATE DEFAULT NULL COMMENT "Actual delivery date"');

CALL add_column_if_not_exists('entries', 'InspectionNotes', '`InspectionNotes` TEXT DEFAULT NULL COMMENT "Inspection notes from Step 4"');
CALL add_column_if_not_exists('entries', 'InspectionDate', '`InspectionDate` TIMESTAMP NULL DEFAULT NULL COMMENT "When inspection was performed"');
CALL add_column_if_not_exists('entries', 'InspectionStatus', '`InspectionStatus` VARCHAR(50) DEFAULT NULL COMMENT "Inspection status (pending, in_progress, completed, rejected)"');
CALL add_column_if_not_exists('entries', 'InspectedBy', '`InspectedBy` INT(11) DEFAULT NULL COMMENT "User ID who performed inspection"');

CALL add_column_if_not_exists('entries', 'FormType', '`FormType` VARCHAR(50) DEFAULT NULL COMMENT "Form type (ics, ppe, none)"');
CALL add_column_if_not_exists('entries', 'FormData', '`FormData` JSON DEFAULT NULL COMMENT "Complete form data from Step 5"');
CALL add_column_if_not_exists('entries', 'FormSubmitDate', '`FormSubmitDate` TIMESTAMP NULL DEFAULT NULL COMMENT "When form was submitted"');
CALL add_column_if_not_exists('entries', 'FormStatus', '`FormStatus` VARCHAR(50) DEFAULT NULL COMMENT "Form status (pending, completed, rejected)"');

CALL add_column_if_not_exists('entries', 'PrId', '`PrId` INT(11) DEFAULT NULL COMMENT "Reference to purchase_requests.id for backward compatibility"');
CALL add_column_if_not_exists('entries', 'WorkflowStep', '`WorkflowStep` INT(1) DEFAULT 1 COMMENT "Current workflow step (1-5)"');
...
-- =====================================================
-- Note: Using INSERT IGNORE to skip duplicates on re-runs
-- This prevents error #1062 when re-executing the migration

INSERT IGNORE INTO entry_step_mapping (step_number, entry_field, field_label, field_type, is_required, description) VALUES
...
-- =====================================================
-- Uses helper procedure to safely add indexes (idempotent)

CALL add_index_if_not_exists('entries', 'idx_pr_id', '`idx_pr_id` (`PrId`)');
CALL add_index_if_not_exists('entries', 'idx_workflow_step', '`idx_workflow_step` (`WorkflowStep`)');
CALL add_index_if_not_exists('entries', 'idx_approval_status', '`idx_approval_status` (`ApprovalStatus`)');
CALL add_index_if_not_exists('entries', 'idx_form_type', '`idx_form_type` (`FormType`)');
CALL add_index_if_not_exists('entry_workflow_status', 'idx_entry_pr', '`idx_entry_pr` (`entry_id`, `pr_id`)');

-- =====================================================
-- 8. Create audit trail for data binding changes
-- =====================================================
...
-- SELECT COLUMN_NAME, COLUMN_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='entries' AND COLUMN_NAME LIKE '%Approval%' OR COLUMN_NAME LIKE '%Delivery%' OR COLUMN_NAME LIKE '%Inspection%' OR COLUMN_NAME LIKE 'Form%';

-- Check new tables created:
-- SHOW TABLES LIKE 'entry_%';

-- Verify entry_workflow_status structure:
-- DESCRIBE entry_workflow_status;

-- Verify entry_step_mapping structure:
-- DESCRIBE entry_step_mapping;

-- Check if FK constraints are active:
-- SELECT CONSTRAINT_NAME, TABLE_NAME, REFERENCED_TABLE_NAME FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE CONSTRAINT_SCHEMA = DATABASE() AND TABLE_NAME LIKE 'entry_%';

-- Check the workflow_data_binding view:
-- SELECT * FROM workflow_data_binding LIMIT 1;

-- Verify all indices were created:
-- SHOW INDEX FROM entries WHERE Key_name LIKE 'idx_%';
-- SHOW INDEX FROM entry_workflow_status WHERE Key_name LIKE 'idx_%';

-- Check mapping configuration:
-- SELECT step_number, COUNT(*) as field_count FROM entry_step_mapping GROUP BY step_number;

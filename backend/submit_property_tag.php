...
try {
    ...
    // Also insert into entries table for viewing/reporting
    $quantity = 1; // Default quantity
    $unitCost = !empty($estimated_cost) ? $estimated_cost : 0;
    $totalCost = $unitCost;
    $amount = $quantity;
    $inventoryItemNo = $property_number; // Use property_number as inventory item number
    $estimatedUsefulLife = ''; // Can be optionally filled
    ...
    if (!$stmt->execute()) {
        // Only log warning if entry already exists (duplicate)
        if (strpos($stmt->error, 'Duplicate') === false) {
            error_log('Warning: Could not insert into entries table: ' . $stmt->error);
        }
    }
    $stmt->close();

...
try {
    ...
    // ── Resolve user ID ───────────────────────────────────────────────────────
    // Priority: body → session → header → none (require explicit auth in production)
    $user_id = null;
    ...
    if (!$user_id || $user_id <= 0) {
        // DEVELOPMENT ONLY: Default test user for testing
        // TODO: Remove this in production – always require authentication
        if (getenv('APP_ENV') !== 'production' && getenv('ALLOW_TEST_USER') === 'true') {
            ...
            jsonError('Unauthorized: User not identified. Please provide user_id in request body or header (X-User-ID).', 401);
        }
    }
    ...
    $wf_sql = '
        INSERT INTO workflow_status (entry_id, pr_no, pr_id, current_step, created_by) 
        VALUES (?, ?, ?, ?, ?)
    ';
    $wf_stmt = $conn->prepare($wf_sql);
    $current_step = 1;
    $wf_stmt->bind_param('isiii', $entry_id, $pr_no, $pr_id, $current_step, $user_id);
    $wf_stmt->execute();
    $wf_stmt->close();

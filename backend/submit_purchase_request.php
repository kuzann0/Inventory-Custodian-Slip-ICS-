...
try {
    ...
    $wf_sql = '
        INSERT INTO entry_workflow_status
            (entry_id, pr_id, pr_no, current_step, step_1_completed, step_1_data)
        VALUES (?, ?, ?, 1, 1, ?)
    ';
    ...
    $wf_stmt->bind_param('iiss', $entry_id, $pr_id, $pr_no, $step_1_data);

    if (!$wf_stmt->execute()) {
        error_log('[submit_purchase_request] execute(workflow): ' . $wf_stmt->error);
        jsonError('Failed to create workflow status record.', 500);
    }
    $wf_stmt->close();

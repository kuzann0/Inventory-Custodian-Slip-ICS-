...
// ─── Items Table ──────────────────────────────────────────────────────────────
function ItemsTable({ items, grandTotal, onRemove, showRemarks, onRemarkChange, onEdit }) {
      ...
      <table className="items-table">
        <thead>
          <tr>
            ...
            {!showRemarks && (onRemove || onEdit) && <th></th>}
          </tr>
        </thead>
        <tbody>
          {items.map((item, idx) => (
            <tr key={item.id}>
              <td className="mono">{item.itemNo ?? (idx + 1)}</td>
...
function NewEntryPR() {
  ...
  useEffect(() => {
    const saved = sessionStorage.getItem('current_pr_no') || sessionStorage.getItem('new_pr_name');
    if (saved) { setPrNo(saved); }
  }, []);
  ...
  // Auto-set Item No. (auto-increment) when not editing
  useEffect(() => {
    if (editingItemId) return;
    const maxNo = particularItems.reduce((m, it) => Math.max(m, Number(it.itemNo || 0)), 0);
    setItemNo(String(maxNo + 1));
  }, [particularItems, editingItemId]);
  ...
  const addItemHandler = () => {
    ...
    // Duplicate description check (allow same when editing same item)
    const isDuplicate = particularItems.some(
      (i) => i.particular.toLowerCase() === inputParticular.trim().toLowerCase() && i.id !== editingItemId
    );
    ...
    if (editingItemId) {
      ...
      const updated = particularItems.map(i => {
        if (i.id === editingItemId) {
          return {
            ...i,
            itemNo:    itemNo ? Number(itemNo) : i.itemNo,
      ...
      setParticularItems(updated);
      setEditingItemId(null);
    } else {
      // Use manual itemNo when provided, otherwise auto-increment
      const maxNo = particularItems.reduce((m, it) => Math.max(m, Number(it.itemNo || 0)), 0);
      const nextNo = maxNo + 1;
      const newItemNo = itemNo ? Number(itemNo) : nextNo;
      const newItem = {
        id:         Date.now(),
        itemNo:     newItemNo,
        particular: inputParticular.trim(),
  ...
  const removeItemHandler = (item_id) => {
    ...
    if (editingItemId === item_id) {
      setEditingItemId(null);
      setInputParticular(''); setInputUnit(''); setInputQuantity(0); setInputUnitCost(0);
    }
  };

  // Edit an existing item: populate the add-row with values
  const handleEditItem = (item_id) => {
    const item = particularItems.find(i => i.id === item_id);
    if (!item) return;
    setEditingItemId(item_id);
    setInputParticular(item.particular || '');
    setInputUnit(item.unit || '');
    setInputQuantity(item.quantity || 0);
    setInputUnitCost(item.unitCost || 0);
    // keep itemNo displayed but editing preserves original itemNo
    setItemNo(item.itemNo ?? '');
    // Scroll to add-row (optional UX)
    const el = document.querySelector('.add-row'); if (el) el.scrollIntoView({behavior:'smooth', block:'center'});
  };
                ...
                <div className="gap-sm">
                  <Field label="Item description" req error={rowErrors.particular}>
                    <div className="add-row">
                      ...
                      />
                      <button type="button" className="btn-add" onClick={addItemHandler} title="Add item">{editingItemId ? 'Save' : '+'}</button>
                      {editingItemId && (
                        <button type="button" className="btn btn-ghost" onClick={() => { setEditingItemId(null); setInputParticular(''); setInputUnit(''); setInputQuantity(0); setInputUnitCost(0); setRowErrors({}); }} style={{marginLeft:8}}>Cancel</button>
                      )}
                    </div>
                  </Field>
                </div>
            ...
            {currentStep === 'approval' && (
              <div>
                ...
                {particularItems.length > 0 && (
                  <ItemsTable items={particularItems} grandTotal={grandTotal} onEdit={handleEditItem} />
                )}
...
<ItemsTable items={particularItems} grandTotal={grandTotal} onRemove={removeItemHandler} onEdit={handleEditItem} />

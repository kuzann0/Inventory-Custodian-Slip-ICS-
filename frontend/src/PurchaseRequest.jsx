...
import PurchaseRequestStep2Preview from './components/PurchaseRequestStep2Preview.jsx';
import NoticeOfDeliveryPreview from './components/NoticeOfDeliveryPreview.jsx';
import IARPreview from './components/IARPreview.jsx';
...
// ─── Step indicator ───────────────────────────────────────────────────────────
const STEPS = [
  { key:'create_and_preview', label:'PR & Preview'  },
  { key:'approval',           label:'Approval'             },
  { key:'delivery_and_nod',   label:'NOD & Preview'       },
  { key:'inspection',         label:'IAR & Preview'           },
  { key:'form_selection',     label:'Form Selection'       },
];
...
function NewEntryPR() {
  ...
  const safePost = async (endpoint, payload, attempt = 1) => {
    ...
    if (!response.ok) {
      // Surface the actual PHP error message instead of swallowing it
      let serverMsg = `Server error (HTTP ${response.status}).`;
      try {
        const errJson = JSON.parse(rawText);
        if (errJson?.error) serverMsg += ' ' + errJson.error;
        if (errJson?.debug) serverMsg += ' Debug: ' + JSON.stringify(errJson.debug);
      } catch (_) {
        if (rawText.trim()) serverMsg += ' Response: ' + rawText.slice(0, 200);
      }
  ...
  // ─── State (unchanged) ─────────────────────────────────────────────────────
  const [currentStep, setCurrentStep] = useState('create_and_preview');
  ...
  const handleCreatePR = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setSubmitMessage(`✓ PR ${finalPrNo} created! Showing preview…`);
  ...
  const handleApprovalDecision = async (decision) => {
    ...
    try {
      if (!prId) {
        throw new Error('PR ID is missing. Please create a PR first.');
      }
      const payload = {
        pr_id: parseInt(prId) || 0,
        action: decision === 'approved' ? 'approve' : 'reject',
        notes: decision === 'approved' ? 'Approved by system' : 'Rejected by reviewer',
        user_id: parseInt(sessionStorage.getItem('user_id')) || 1,
        approver_name: (approverName || 'System').trim(),
        approver_position: (approverPosition || 'Officer').trim(),
        approver_office: (approverOffice || 'MARINA').trim(),
      };
      ...
      if (decision === 'disapproved') {
        if (result.success) {
          ...
          setTimeout(() => {
            ...
            setCurrentStep('create_and_preview'); setApprovalDecision(null); setSubmitMessage('');
          }, 2000);
        } else {
        ...
        if (result.success) {
          ...
          setMessageType('success');
          setTimeout(() => { setSubmitMessage(''); setCurrentStep('delivery_and_nod'); }, 1500);
        } else {
  ...
  // ─── Step 3: Delivery Note (auto-show NOD preview) ─────────────────────────
  const handleDeliveryNote = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        setSubmitMessage('✓ Delivery data saved! Showing preview…');
        setMessageType('success');
        setShowPreview(true);
        setTimeout(() => { setSubmitMessage(''); }, 2000);
      } else {
  ...
  // ─── Step 4: Inspection (show form, then preview) ──────────────────────
  const handleInspection = async (e) => {
    ...
    setIsSubmitting(true);
    try {
      // Just show the preview - don't submit yet
      setShowPreview(true);
      setSubmitMessage('');
      setIsSubmitting(false);
    } catch (error) {
  ...
  // ─── NEW: Confirm IAR Preview & Submit ─────────────────────────────────
  const handleIARConfirm = async () => {
    setIsSubmitting(true);
    setSubmitMessage('');
    try {
      const payload = {
      ...
      if (result.success) {
        setSubmitMessage('✓ IAR confirmed. Proceeding to form selection…');
        setMessageType('success');
        setTimeout(() => {
          setSubmitMessage('');
          setShowPreview(false);
          setCurrentStep('form_selection');
        }, 1500);
      } else {
  ...
  // ─── Navigation back ──────────────────────────────────────────────────────
  const goBack = () => {
    const steps = ['create_and_preview','approval','delivery_and_nod','inspection','form_selection'];
  ...
  // ─── Preview Complete Handler ────────────────────────────────────────────
  const handlePreviewComplete = () => {
    setSubmitMessage('✓ Preview confirmed. Proceeding to approval…');
    setMessageType('success');
      ...
      <div className="pr-shell">
        <div className="pr-card">
          ...
          <div className="pr-body">
            ...
            {/* STEP 1 – Create PR & Preview ────────────────────────────────────────────────────── */}
            {currentStep === 'create_and_preview' && !showPreview && (
              <form onSubmit={handleCreatePR}>
                ...
                <Field label="Certification">
                  ...
                  <p style={{fontSize:'12px', color:'var(--text-3)', marginTop:'6px'}}>Optional - Select to indicate budget availability status</p>
                </Field>
                ...
                <div className="pr-footer">
                  <div style={{ display: 'flex', gap: '8px' }}>
                    ...
                    <button className="btn btn-primary" type="submit" disabled={isSubmitting}>
                      {isSubmitting ? <><Spinner /> Creating…</> : 'Create & Preview →'}
                    </button>
                  </div>
                </div>
              </form>
            ...
            {/* STEP 1 – Preview PR (Marina form template with PDF download) */}
            {currentStep === 'create_and_preview' && showPreview && (
              <PurchaseRequestStep2Preview
                office={office}
                ...
                onConfirm={handlePreviewComplete}
              />
            )}
              ...
              <div>
                ...
                <div className="decision-area">
                  <div className="decision-btns">
                    ...
                    <button className="btn-danger" onClick={() => handleApprovalDecision('disapproved')} disabled={isSubmitting}>
            ...
            {/* STEP 3 – Delivery & NOD Preview */}
            {currentStep === 'delivery_and_nod' && !showPreview && (
              <form onSubmit={handleDeliveryNote}>
                ...
                <div className="pr-footer">
                  <button type="button" className="btn btn-ghost" onClick={goBack} disabled={isSubmitting}>← Back</button>
                  <button className="btn btn-primary" type="submit" disabled={isSubmitting}>{isSubmitting ? <><Spinner /> Saving…</> : 'Preview NOD →'}</button>
                </div>
              </form>
            )}
            ...
            {/* STEP 3 – NOD Preview */}
            {currentStep === 'delivery_and_nod' && showPreview && (
              <NoticeOfDeliveryPreview
                supplier={supplier}
                prNo={prNo}
                prDate={dateRequested}
                siNo={siNo}
                drNo={drNo}
                deliveryDate={poDate}
                particularItems={particularItems}
                ...
                onBack={() => setShowPreview(false)}
                onConfirm={() => {
                  setSubmitMessage('✓ NOD confirmed. Proceeding to inspection…');
                  setMessageType('success');
                  setShowPreview(false);
            ...
            {/* STEP 4 – Inspection (form or preview) */}
            {currentStep === 'inspection' && !showPreview && (
              <form onSubmit={handleInspection}>
                ...
                <div className="pr-footer">
                  <button type="button" className="btn btn-ghost" onClick={goBack} disabled={isSubmitting}>← Back</button>
                  <button className="btn btn-primary" type="submit" disabled={isSubmitting}>{isSubmitting ? <><Spinner /> Saving…</> : 'Preview IAR →'}</button>
                </div>
              </form>
            )}
              ...
              <>
                {submitMessage && (
                  <div className={`toast ${messageType}`}>
                    {messageType === 'success' ? '✓' : '⚠'} {submitMessage}
                  </div>
                )}
                <IARPreview
                  prNo={prNo}
                  supplier={iacSupplier}
                  iacIarNo={iacIarNo}
                  iacDate={iacDate}
                  iacInvoiceNo={iacInvoiceNo}
                  iacInvoiceDate={iacInvoiceDate}
                  iacPoDate={iacPoDate}
                  iacRequisitioningOffice={iacRequisitioningOffice}
                  iacRequisitioningCode={iacRequisitioningCode}
                  particularItems={particularItems}
                  totalAmount={totalAmount}
                  inspectionNotes={inspectionNotes}
                  onBack={() => setShowPreview(false)}
                  onConfirm={handleIARConfirm}

...
function SuperAdminPage() {
  ...
  const loadCapabilities = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/get_capabilities.php?requester_role_id=${roleId}`
      );
      const data = await response.json();
  ...
  const loadAuditLogs = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:8080/get_audit_logs.php');
      const data = await response.json();
  ...
  const handleCreateUser = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await fetch('http://localhost:8080/create_user.php', {
        method: 'POST',
  ...
  const handleUpdateUser = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch('http://localhost:8080/update_user.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          actor_id: actorId,
  ...
  const handleDeleteUser = async (userId, username) => {
    ...
    try {
      setLoading(true);
      const response = await fetch('http://localhost:8080/delete_user.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          actor_id: actorId,
